...
* @Description: TODO(自定义TextView 用于控制换行)
* @author ZhuTao
...
public class MTextView extends TextView {
   ...
   /**
    * 是否使用默认{@link TextView#onMeasure(int, int)}和
    * {@link TextView#onDraw(Canvas)}
    */
   ...
   protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
       ...
       height += getCompoundPaddingTop() + getCompoundPaddingBottom() + 0;	//+10是因为绘制不全，差一点点
       ...
       for (int i = 0; i < contentList.size(); i++) {
           ...
           for (int j = 0; j < line.line.size(); j++) {
               ...
               if (ob instanceof String) {
                   canvas.drawText((String) ob, realDrawedWidth, height
                           + line.height, paint);
   ...
   public void setMText(CharSequence cs) {
       ...
       if (cs instanceof SpannableString) {
           ...
           ImageSpan[] imageSpans = ss.getSpans(0, ss.length(),
                   ImageSpan.class);
